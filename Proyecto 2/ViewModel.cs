...
public class CalculadoraPropinaViewModel : ObservableObject
{
    ...
    {
        SeleccionarPropinaCommand = new RelayCommand<object?>(SeleccionarPropina);
        CambiarCantidadPersonasCommand = new RelayCommand<object?>(CambiarCantidadPersonas);
        Recalcular();
    }
    ...
    public IRelayCommand<object?> SeleccionarPropinaCommand { get; }
    public IRelayCommand<object?> CambiarCantidadPersonasCommand { get; }

    private void SeleccionarPropina(object? parameter)
    {
        if (TryGetInt(parameter, out int porcentaje))
        {
            PorcentajePropina = porcentaje;
        }
    }

    private void CambiarCantidadPersonas(object? parameter)
    {
        if (TryGetInt(parameter, out int cambio))
        {
    ...
    {
        if (parameter is int entero)
        {
            value = entero;
            return true;
        }

        if (parameter is string texto && int.TryParse(texto, NumberStyles.Integer, CultureInfo.CurrentCulture, out value))
        {
            return true;
        }

        if (parameter is string textoInvariant && int.TryParse(textoInvariant, NumberStyles.Integer, CultureInfo.InvariantCulture, out value))
        {
            return true;
        }
